...
target_image_path=os.path.join(DATA_DIR,"output_reference","CCF_100048576_205.png")
SAVE_DIR="../../output"
SAVE_PATH=os.path.join(SAVE_DIR,"h5ad_2_img","PUCK29")
...
# =========================
# 步骤三：读取结果，并与彩色 Target Image 叠加预览
# =========================
print("\n🚀 步骤三 正在生成彩色叠加对比图...")
...
aligned_h5ad_img = cv2.imread(transformed_h5ad_img_path, cv2.IMREAD_GRAYSCALE)
...
if target_img is None or aligned_h5ad_img is None:
    ...
    # 5. 保存结果
    overlay_save_path = os.path.join(save_path_alignment, "color_alignment_overlay.png")
    cv2.imwrite(overlay_save_path, overlay_img)
    
    # 同时保存一份原图和叠加图的左右对比
    comparison = np.hstack((target_img, overlay_img))
    cv2.imwrite(os.path.join(save_path_alignment, "color_side_by_side.png"), comparison)
